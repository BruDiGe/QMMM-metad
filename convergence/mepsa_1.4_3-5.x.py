...
def general_canvas(figsize, dpi):
    ...
    mpl.rc('font', family='DejaVu Sans', size=15)
...
# Function modifications for omitting "MIN AND FLAT" plotting
    ...
    if data_type == "list":
        ...
        for arg in args:
            ...
            for points_list in arg:
                ...
                if point_set_counter == 2:  # Skipping the second set of points (assumed to be "MIN AND FLAT")
                    continue
                if (len(in_color)>1):
        ...
        for arg in args:
            temp_arg = numpy.asarray(arg)
            if temp_arg.shape[0] == 2:  # Skipping "MIN AND FLAT"
                continue
            x = temp_arg.transpose()[0]
            y = temp_arg.transpose()[1]
            plt.scatter(x,y,color=in_color[0] if len(in_color) == 1 else in_color[color_counter], alpha=0.5)
            color_counter += 1
